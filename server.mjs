...
app.post("/api/resume/assist",async(req,res,next)=>{try{
  ...
  const facts={name:clip(data.name,150),englishGivenName:clip(data.englishGivenName,100),englishSurname:clip(data.englishSurname,100),role:clip(data.role,150),email:clip(data.email,200),phone:clip(data.phone,100),city:clip(data.city,120),linkedin:clip(data.linkedin,350),summary:clip(data.summary,1500),skills:clip(data.skills,1500),education:clip(data.education,1500),languages:(Array.isArray(data.languages)?data.languages:[]).slice(0,10).map(item=>({language:clip(item.language,80),level:clip(item.level,80)})).filter(item=>item.language&&item.level),experience:(Array.isArray(data.experience)?data.experience:[]).slice(0,10).map(item=>({id:clip(item.id,80),kind:clip(item.kind,30),position:clip(item.position,150),company:clip(item.company,150),city:clip(item.city,100),start:clip(item.start,50),end:clip(item.end,50),duties:clip(item.duties,1500)}))};
  ...
  const formats={summary:'{"summary":"2–4 concise first-person sentences"}',skills:'{"hardSkills":["..."],"softSkills":["..."]}',functions:'{"functions":["5–7 concise responsibility or achievement statements"]}',adapt:'{"summary":"adapted first-person summary","skills":["6–9 relevant skills"],"experience":[{"id":"original experience id","functions":["3–6 vacancy-relevant statements"]}]}',proofread:'{"role":"corrected desired role","city":"corrected city name in Ukrainian","summary":"corrected first-person summary","skills":"corrected skills text","education":"corrected education text","experience":[{"id":"original experience id","position":"corrected position title","city":"corrected city name in Ukrainian","duties":"corrected duties preserving line breaks"}]}',english_cv:'{"name":"supplied given name followed by supplied surname","role":"professional English job title","email":"unchanged email","phone":"unchanged phone","city":"city in English","linkedin":"unchanged LinkedIn","summary":"professional summary in first person","skills":"English skills separated by new lines","education":"education in English","languages":[{"language":"language in English","level":"exact supplied level"}],"experience":[{"id":"original experience id","kind":"original kind","position":"English position title","company":"company name appropriately transliterated or translated","city":"city in English","start":"English date","end":"English date or Present","duties":"English duties preserving line breaks"}]}'};
  if(!formats[action])return res.status(400).json({error:"Невідома дія AI."});
  const instructions={summary:"Write a concise professional About Me section in the candidate's first person based only on supplied facts and desired role.",skills:"Suggest only 6–9 genuinely relevant skills. Avoid a generic pile of clichés. Do not claim certifications, tools or abilities unsupported by the facts.",functions:"Suggest typical, truthful responsibility formulations for the supplied position. Phrase them as editable suggestions; do not invent numbers, employers, awards, ranks or operations.",adapt:"Adapt the whole resume to the vacancy: summary, skills, and duties for every experience item. Keep company, role and dates unchanged. Reorder, omit or rephrase duties to emphasize relevant facts, but never invent experience, achievements, numbers or tools. Return every experience item using its original id.",proofread:"Proofread the supplied Ukrainian resume. Correct spelling, punctuation, grammar, word agreement and obvious typing errors only. Normalize city names to their standard Ukrainian spelling (for example, 'Днепр' to 'Дніпро'). Preserve every fact, number, proper name, employer, role, date and the meaning. Do not add, remove, embellish or adapt content. Keep the summary in the first person and preserve duty line breaks and bullet structure. Return every experience item using its original id.",english_cv:"Create an ATS-friendly International English CV in clear professional British English. Adapt terminology naturally rather than translating word for word. Create the display name as englishGivenName followed by englishSurname. Preserve the exact Latin spelling supplied by the user; only change all-uppercase input to conventional name case. Never invent, translate, reorder or transliterate the name yourself. Keep email, phone and LinkedIn unchanged. Transliterate Ukrainian city names using accepted English spellings (for example, Дніпро to Dnipro and Київ to Kyiv). Use Month YYYY date style and Present for current roles. Keep Professional Summary to one compact paragraph of 3–5 sentences. Return skills in consistent sentence case, one skill per line. Return supplied languages in a separate languages array and never place language levels inside the summary. Translate education naturally and avoid awkward phrases such as Specialist Degree / Higher Education when Specialist Degree is sufficient. Preserve every fact, employer, date and experience item; do not invent achievements, years of experience, numbers, qualifications, tools or language levels. Return every experience item using its original id and preserve duties as separate lines."};
  const editorRole=action==="english_cv"
    ?"You are a meticulous British English CV editor. Respond only in polished, grammatically correct professional British English."
    :"You are a careful Ukrainian career editor. Respond in grammatically correct Ukrainian.";
  const firstPersonRule=action==="english_cv"
    ?"Write the Professional Summary strictly in the candidate's first person. Check and correct English spelling, grammar, punctuation, word choice, capitalization and obvious typing errors in every generated field before answering."
    :"Write the About Me section strictly in first person (for example: \"організовую\", \"володію\", \"дотримуюся\"), never as an outside description (\"організовує\", \"володіє\"). Proofread grammar before answering.";
  const prompt=`${editorRole} Return ONLY valid JSON in this exact shape: ${formats[action]}. ${instructions[action]} ${firstPersonRule} Preserve the user's facts and intended meaning. The user will review and edit every suggestion. Profile: ${clip(profile,40)}. Candidate facts: ${JSON.stringify(facts)}. Selected experience: ${JSON.stringify(selected)}. Vacancy: ${clip(vacancy,6000)||"not supplied"}. For military experience, use the Ukrainian veteran-employment principle of translating actual functions into understandable civilian competencies. Treat the military title only as a clue, rely on the user's stated facts, do not disclose unit numbers, locations, operations, weapons details or other sensitive information. For medical experience, preserve accurate terminology and never invent procedures or certifications.`;
  const ai=new GoogleGenAI({apiKey:process.env.GEMINI_API_KEY});
