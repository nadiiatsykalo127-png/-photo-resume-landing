...
app.post("/api/resume/assist",async(req,res,next)=>{try{if(!process.env.GEMINI_API_KEY)return res.status(503).json({error:"AI-підказки будуть доступні після підключення ключа."});const ai=new GoogleGenAI({apiKey:process.env.GEMINI_API_KEY});const {profile,data,vacancy}=req.body||{};const prompt=`You are a Ukrainian career editor. Return only valid JSON with keys summary and skills. Improve the resume without inventing facts. Profile: ${profile}. Candidate data: ${JSON.stringify(data)}. Vacancy: ${vacancy||"not provided"}. For military experience, translate responsibilities into respectful civilian competencies. For medical experience, preserve accurate terminology. summary is concise Ukrainian text; skills is an array of 6-10 Ukrainian strings.`;const result=await ai.models.generateContent({model:process.env.GEMINI_TEXT_MODEL||"gemini-3.7-flash",contents:prompt,config:{responseMimeType:"application/json"}});res.type("json").send(result.text)}catch(error){next(error)}});
