...
function normalize(input={}){
  return {
    profile:clip(input.profile,40), language:input.language==="en"?"en":"uk", template:input.template==="standard"?"standard":"stylish",
...
export async function buildDocx(raw){
  const data=normalize(raw),photo=await resumePhoto(data.photo,300,400),stylish=data.template==="stylish",en=data.language==="en";
  const headerText=[];
  headerText.push(new Paragraph({children:[docRun(data.name||(en?"CV":"Резюме"),{size:34,bold:true,color:stylish?WHITE:DARK})],spacing:{after:80},keepNext:true}));
  ...
  if(data.summary){children.push(docHeading(en?"Professional Summary":"Про себе"),docBody(data.summary));}
  if(data.experience.length){
    children.push(docHeading(en?"Work Experience":"Досвід роботи"));
    for(const item of data.experience){
      children.push(docBody(item.position||(en?"Experience":"Досвід"),{bold:true,before:90,after:30,keepNext:true}));
      ...
      if(item.duties.length)children.push(docBody(en?"Responsibilities and Achievements:":"Обов’язки та досягнення:",{bold:true,before:30,after:35,keepNext:true}));
...
export async function buildPdf(raw){
  ...
  const doc=new PDFDocument({size:"A4",margins:{top:42,right:48,bottom:42,left:48},info:{Title:en?"CV":"Резюме",Author:"nadinartdigital.com.ua"}});
  ...
  doc.font("ResumeBold").fontSize(23).fillColor(stylish?"#FFFFFF":`#${DARK}`).text(data.name||(en?"CV":"Резюме"),textX,textY,{width:width-(textX-left)-22});
  ...
  if(data.summary){pdfSection(doc,en?"Professional Summary":"Про себе");doc.font("Resume").fontSize(9.4).fillColor(`#${DARK}`).text(data.summary,{width,lineGap:3});}
  if(data.experience.length){
    pdfSection(doc,en?"Work Experience":"Досвід роботи");
    for(const item of data.experience){
      ensurePdf(doc,50);doc.font("ResumeBold").fontSize(9.7).fillColor(`#${DARK}`).text(item.position||(en?"Experience":"Досвід"),{width});
      ...
      doc.moveDown(.25);
      if(item.duties.length){ensurePdf(doc,24);doc.font("ResumeBold").fontSize(8.8).fillColor(`#${DARK}`).text(en?"Responsibilities and Achievements:":"Обов’язки та досягнення:",{width});doc.moveDown(.15);}
